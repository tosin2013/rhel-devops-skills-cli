...
- Always deploy the hub cluster BEFORE student clusters when using RHACM topology
- Cloud provider quotas must be checked BEFORE provisioning — AWS defaults to 5 VPCs per region, which limits student count
- Student GUIDs must follow the pattern `<base-guid>-sN` (e.g., `demo1-s1`, `demo1-s2`)
- Cross-cluster Showroom terminal config requires the hub API URL and token — these come from `agnosticd_user_info`, not from manual kubeconfig
- `NUM_STUDENTS` defaults to 2, not 1 — always confirm the intended count before provisioning
...
> (RESEARCH NEEDED — RQ-HUB-4)

---

## Phase 2 — Hub Cluster Provisioning

> (RESEARCH NEEDED — RQ-HUB-1)
...
> (RESEARCH NEEDED — RQ-HUB-6)

> (RESEARCH NEEDED — RQ-HUB-3)
...
### RHACM registration during student cluster provisioning

When the hub cluster runs RHACM, each student cluster must be registered to it as a `ManagedCluster`. This is handled by an `ocp4_workload_rhacm_import` role declared in the student cluster vars file — **not** by the deploy script directly.

**Important:** No RHACM import role exists in upstream [agnosticd/agnosticd-v2](https://github.com/agnosticd/agnosticd-v2) (confirmed by inspection of `ansible/roles/`). You must create this role in your own fork.

Scaffold the role under `ansible/roles/ocp4_workload_rhacm_import/` in your fork:
...
Wire into student cluster vars:
...
Use the `deploy.sh` script (generated by the **agnosticd** skill's "Independent Deployment Scripts" section) with `DEPLOY_HUB=true` to ensure the hub provisions before student clusters.
